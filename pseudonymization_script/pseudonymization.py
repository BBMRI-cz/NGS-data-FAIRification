...
class Pseudonimizer:
    ...
    prefix = "{http://www.bbmri.cz/schemas/biobank/data}"
    ...
    def __call__(self):
        self.pseudonimize_run()


    def pseudonimize_run(self):
        clinical_data, predictive_pseudo_tuples = self.pseudo_sample_sheet()
        self.saveClinicalFiles(clinical_data)
    ...
    def check_for_predictive_number_in_export(self, predictive_number, pseudo_number):
        ...
        for export in os.listdir(self.export_path):
            ...
            root = tree.getroot()
            lts = root.find(f"{self.prefix}LTS")
            for child in lts:
    ...
    def saveClinicalFiles(self, clinical_data_list):
    ...
    def prepare_tissue(self, lts_child, pseudo_number):
        info = {
            ...
            "sampleNumber": lts_child.find(f"{self.prefix}samplesNo").text,
            "availableSamplesNumber": lts_child.find(f"{self.prefix}availableSamplesNo").text,
            "materialType": lts_child.find(f"{self.prefix}materialType").text,
            "pTNM": lts_child.find(f"{self.prefix}pTNM").text,
            "morphology": lts_child.find(f"{self.prefix}morphology").text,
            "diagnosis": lts_child.find(f"{self.prefix}diagnosis").text,
            "cutTime": lts_child.find(f"{self.prefix}cutTime").text,
            "freezeTime": lts_child.find(f"{self.prefix}freezeTime").text,
            "retrieved": lts_child.find(f"{self.prefix}retrieved").text,
            }
        return info
    ...
    def prepare_serum(self, lts_child, pseudo_number):
        info = {
            ...
            "availableSamplesNumber": lts_child.find(f"{self.prefix}availableSamplesNo").text,
            "materialType": lts_child.find(f"{self.prefix}materialType").text,
            "takingDate": lts_child.find(f"{self.prefix}takingDate").text
        }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Pseudonimized run and adds clinical data to it")
    ...
    Pseudonimizer(args.run, args.export, args.pseudo)()